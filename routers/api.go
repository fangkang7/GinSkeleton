...
func InitApiRouter() *gin.Engine {
	...
	api := router.Group("/go")
	{
		...
		home := api.Group("home/")
		{
			...
			// 3.将以上验证器注册在容器：app/http/validator/common/register_validator/api_register_validator.go  18 行为注册时的键（consts.ValidatorPrefix + "HomeNews"）。那么获HomeNews取的时候就用该键即可从容器获取
